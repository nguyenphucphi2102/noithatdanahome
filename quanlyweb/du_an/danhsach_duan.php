			...
			<div class="breadcrumb-wrapper d-flex align-items-center justify-content-between">
				<div>
					...
					<p class="breadcrumbs"><span><a href="index.html">Trang chủ</a></span>
						...
						<span><i class="mdi mdi-chevron-right"></i></span>Danh sách dự án
					</p>
				</div>
			</div>
			<div class="row">
					...
					function pageList($curpage, $pages)
					{
						...
						if ($curpage != 1 && $curpage) {
							$page_list .=
								"<a href=\"" .
								$_SERVER["PHP_SELF"] .
								"?p=danhsach_duan&page=1\" title=\"Trang đầu\"> << </a>";
						}
				...
				$count = mysqli_num_rows(mysqli_query($link, "SELECT*FROM du_an"));
				$pages = $p->findPages($count, $limit);
				$result = mysqli_query($link, "SELECT * FROM du_an ORDER BY id DESC limit $start,$limit");
				$stt = 1;
				?>
								...
								<table id="responsive-data-table" class="table" style="width:100%">
									...
									<tbody class="text-center">
										...
										while ($row = mysqli_fetch_object($result)) {
											...
											$did = $row->thuocloai;
											$sql3 = mysqli_query($link, "select * from du_an where id='" . $did . "'");
											...
											<tr>
												...
												<td>
													<a href="quan_tri.php?p=xoa_duan&id=<?= $id ?>"
												...
												<td>
													<a
														href="quan_tri.php?p=sua_duan&id=<?= $id ?>&thuocloai=1&page=<?= $_GET['page'] ?>">
