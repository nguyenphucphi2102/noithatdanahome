...
$did  = $_GET["url"];
$tv   = "SELECT * FROM loai_tin_sanphama WHERE linkurl = '" . mysqli_real_escape_string($link, $_GET['url']) . "' ORDER BY id";
...
<section class="cat-news-wrapper tp-section-gap mt-4">
    <div class="container">
        ...
        <div class="row g-4">
            ...
            $stmt = $link->prepare("SELECT COUNT(*) FROM tin_sanphama WHERE thuocloai = ?");
            ...
            $stmt = $link->prepare("SELECT * FROM tin_sanphama WHERE thuocloai = ? ORDER BY id DESC LIMIT ?, ?");
